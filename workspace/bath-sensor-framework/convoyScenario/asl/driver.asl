...
+updatedTempPositionHolder(PosX, PosY, PosZ, Health, Speed, ChassiHeading, AuxInfo) : true 
																									<- //.print("not moved");
																										...
																										//VB DANGER this might not be backwards compatible, but should work OK with SUMO updates
...
+entityUpdate(PosX, PosY, PosZ, Health, Speed, ChassiHeading, AuxInfo) : currentRequestedSpeed(SpeedVal) & started(Owner) & collisionMinTime(CTime)
									& geom.convert(SpeedVal,CTime,ChassiHeading, PosX,PosY,PosZ,XFIN,YFIN,ZFIN) //& geom.calcVolume(PosX,PosY,PosZ,ChassiHeading,Xi,Yi)	
									<-// .print("driver told entityUpdate ",PosX," ",PosY, " ",PosZ);
									// .abolish(\+entityUpdate(PosX, PosY, PosZ, Health, Speed, ChassiHeading, AuxInfo));
									//.wait(250);
...
+!arrivedAtDestination : true & started(Owner) <- //.print("XXXXXX arrived XXXXXXXXX");
					.send(Owner, achieve, chosenSpeed(0));
					.abolish(currentRequestedSpeed(_));
					+currentRequestedSpeed(0);
					//print("stopped");
...
+!moveToKnownPosition : entityUpdate(X,_,Z,_,_,Heading,_) 
					...
					& distanceRemaining(MIN,MAX,GOAL) & MIN <= D1 & MAX >= D1 
								<- 
							//.print("using moveTo based on SUMO plan, ",D1);
							.wait(250);
