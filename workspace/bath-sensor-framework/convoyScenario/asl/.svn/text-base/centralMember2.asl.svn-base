//any initial beliefs
terrain(desert).
convoyAgentName(convoyMember2).
driverAgentName(driver2).
...
+detectionInCollisionZone(Name, Distance) : Distance <=46  <- .print("vehicle ",Name," detected too close in collision zone!! distance is ", Distance);
													.abolish(collisionVolClear);
													!brakeHard(Name).

+detectionInCollisionZone(Name, Distance) : Distance > 46 & Distance <=65 <- .print("vehicle ",Name," detected near in collision zone!! distance is ", Distance);
													.abolish(collisionVolClear);
													!flashLights(Name).		
...
//+time(CurrentSimTime) : .count(time(_),N) <- .print("been told ",N, " times, currrent of ", CurrentSimTime).
...
+!startUsePull : true & convoyAgentName(CAgentName)  & driverAgentName(DAgentName) <- .print("started data pull scenario");
						...
						.send(CAgentName, tell, vehicleAhead(convoyMember1));
						.send(CAgentName, tell, myDriverName(DAgentName));
						.send(CAgentName, tell, started(N));
						.wait(500).
...
+!startUseInstFlash : convoyAgentName(CAgentName)  & driverAgentName(DAgentName) & targetSpeed(V) & insertTime(T)
						...
						//do this in the other section now too
						//.wait(2000);
						//.print("sending speed message to ", V);
						//.send(DAgentName, achieve, setSpeed(V));
