...
public class VehicleArch extends AgArch {
	...
	//private WorkerInstSender instSender;
	private WorkerJStateSender myJStateSender;
	private static String XMPPServer = "127.0.0.1";
	//private Rectangle2D myCollisionVolume = new Rectangle2D.Double();
	...
	public void init() 
	{
		...
		//System.out.println("init started as agent " + this.getAgName());
	...
	public boolean checkCollisionVolume(Term[] terms) 
	{
		if (newBV != null)
		{
			...
			//Vector3f checkPos = new Vector3f(xLoc,0f,zLoc);
			//if (myName.equals("centralMember2"))
			//{
		//		System.out.println(myName + " startat " + myStart.toString() + ", checking if collision vol at " + newBV.getCenter().toString() + " which is " + myStart.distance(newBV.getCenter()) + "m away, and volume contains " + otherVehName + " at " + xLoc + ", " + zLoc + " which is " + myStart.distance(otherLocVec) + "m");
			
				
		//		System.out.println(myName + " vol contains " + otherLocVec.toString() + "?: " + newBV.contains(otherLocVec) + " distance is " + newBV.distanceTo(otherLocVec) + " from centre, nearest edge is " + newBV.distanceToEdge(otherLocVec));
		//	}
