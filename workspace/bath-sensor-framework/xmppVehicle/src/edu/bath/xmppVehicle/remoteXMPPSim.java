...
public class remoteXMPPSim extends Sensor {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 0)
		{
			...
			remoteXMPPSim ps = new remoteXMPPSim(XMPPServer, "test1-vehicle", "jasonpassword", jasonSensorVehicles , "http://127.0.0.1/vehicleSensors", "http://127.0.0.1/vehicleSensors/test1-vehicle");
			vehicleName="central";
		...
		if (args.length == 1)
		{
			...
			remoteXMPPSim ps = new remoteXMPPSim(XMPPServer, args[0]+"-vehicle", "jasonpassword", jasonSensorVehicles , "http://127.0.0.1/vehicleSensors", "http://127.0.0.1/vehicleSensors/"+args[0]+"-vehicle");
			vehicleName=args[0];
			...
			else if (vehicleName.indexOf("2") > 0)
			{
				...
				/*currentXYZLocation.x = -20.00f;
				currentXYZLocation.y = 0f;
				currentXYZLocation.z = 30.00f;*/
		...
		{
			//remoteXMPPSim ps = new remoteXMPPSim("me-pc", "central", "jasonpassword", "jasonSensor", "http://127.0.0.1/sensors/locations/here", "http://127.0.0.1/sensors/locations/here/0EAS");
			remoteXMPPSim ps = new remoteXMPPSim(XMPPServer, args[0]+"-vehicle", args[1], args[2], args[3], args[4]);
			vehicleName=args[0];
			System.out.println("Created jasonSensor, now entering its logic!");
			ps.run();
		}
	...
	public void run() throws XMPPException {
		...
		while(sensorClient == null) {
			try {
				sensorClient = new SensorClient(XMPPServer, vehicleName+"-vehicle-receiver", "jasonpassword");
...
//				long time = System.currentTimeMillis();
			...
			//	List<String> messageStore = new ArrayList<String>(100);
				while(alive) {
					...
					try {
						...
						try {
						//	System.out.println("pri handle " + getPrimaryHandle() + " and loc " + getCurrentLocation());
							DataReading testReading = new DataReading(getPrimaryHandle(), getCurrentLocation(), System.currentTimeMillis());
							//testReading.setType("http://127.0.0.1/types#Position");
							...
							//testReading.addDataValue("http://127.0.0.1/entityDefinitions/vehicle", "http://127.0.0.1/sensors/types#spatial", spatialInfo, false);
